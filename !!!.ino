...
// GPIO 22 занят I2C SCL, кнопка старта перенесена на GPIO 34.
static const uint8_t START_BTN_PIN = 34;
...
void selectIncubationProfileOnBoot() {
  // При включении с нажатой кнопкой GPIO34 переключаем таблицу инкубации.
...
void readActuators(bool& heaterOn, bool& humidifierOn, bool& fanAuto) {
  heaterOn = lastSensorValid && (ui.temperature < (curDayProg.targetTemp - 0.2f));
...
void drawDateTimeBlock(const char* s) {
  ...
  tft.setTextColor(DATE_COLOR, BG_COLOR);
  tft.drawString(s, 240, 67, 2);
}
...
void drawDayInfoBlock(int day, int totalDays, int toHatch) {
  ...
  drawFieldLabel(currentProfile == INCUB_PROFILE_PEREPEL ? "PROFILE:PEREPEL" : "PROFILE:CHICK", 300, 208, DATE_COLOR, 1);
}
